...
void OnChatReady(RakNet::Packet* packet)
{
	RakNet::BitStream bs(packet->data, packet->length, false);
	RakNet::MessageID messageId;
	bs.Read(messageId);
...
void InputHandler()
{
	while (isRunning)
	{
		...
		else if (g_networkState == NS_Pending)
		{
			...
			doOnce = true;
			g_networkState = NS_ChatRoomStart;
			
		}
		else if(g_networkState==NS_ChatRoomStart)
		{
			RakNet::BitStream bs1;
			bs1.Write((RakNet::MessageID)ID_GAMECHAT_START);
			static bool doOnce = false;
			if (!doOnce) {
			...
			RakNet::BitStream bs;
			bs.Write((RakNet::MessageID)ID_THEGAME_LOBBY_READY);
			RakNet::RakString message(userInput);
			bs.Write(message);
		}
